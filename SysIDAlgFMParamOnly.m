%% IRLS Algorithm for only motor, friction and offset parameters
...
function [X, Wfull, alphanew] = SysIDAlgFMParamOnly(variables,AlgOptions,na_idx,nu_idx,J,lb,ub,lba,uba,X_ip,T,data,dataFull,W_ip,X0_1,options)
...
if SearchAlpha
    [X, Wfm, alphanew] = AlgAlphaFMOnly(T_hat,tol,p_fm,n,ncv,na,na_idx,nu_idx,m,k,MS1,MS2,data,dataFull,J,lowerB1,upperB1,lowerB2,upperB2,X0_1,includeOffset,includeConstraints,constraintVariant,options);
else
    [X, Wfm] = AlgNoAlphaFMOnly(T_hat,tol,p_fm,n,ncv,na,na_idx,nu_idx,m,k,MS1,data,dataFull,J,lowerB1,upperB1,X0_1,includeOffset,includeConstraints,constraintVariant,options);
    alphanew = ones(n,1);
end
...
function [X, Wfm, alphanew] = AlgAlphaFMOnly(T_hat,tol,p_fm,n,ncv,na,na_idx,nu_idx,m,k,MS1,MS2,data,dataFull,J,lowerB1,upperB1,lowerB2,upperB2,X0_1,includeOffset,includeConstraints,constraintVariant,options)
...
alphaold = zeros(n,1);
    ...
    while(norm(alphaold-alphanew)>tol)
        ...
        while(norm(Dold-Dnew)>tol)
            ...
            while(norm(Oold-Onew)>tol)   
                ...
                for i = 1:m
                    Ts((i-1)*ncv+1:i*ncv,:) = O_sqrt\T_hat((i-1)*ncv+1:i*ncv);
                    Wfms((i-1)*ncv+1:i*ncv,:) = O_sqrt\Wfm((i-1)*ncv+1:i*ncv,:);
    ...
    if includeOffset
        ...
        Fest = (Th - Wfmh*[X(1:n)' zeros(1,3*n)]');
        ...
        Fest = (Th - Wfmh*[X(1:n)' zeros(1,2*n)]');
        % get intial conditions for friction and friction exponent optimization problem
        X0_2 = [alphanew; X(n+1:end)];
    end
...
function [X, Wfm] = AlgNoAlphaFMOnly(T_hat,tol,p_fm,n,ncv,na,na_idx,nu_idx,m,k,MS1,data,dataFull,J,lowerB1,upperB1,X0_1,includeOffset,includeConstraints,constraintVariant,options)
    ...
    while(norm(Dold-Dnew)>tol)
        ...
        while(norm(Oold-Onew)>tol)   
            ...
            % compute normalized observation matrix and torque vector
            for i = 1:m
                Ts((i-1)*ncv+1:i*ncv,:) = pinv(O_sqrt)*T_hat((i-1)*ncv+1:i*ncv);
                Wfms((i-1)*ncv+1:i*ncv,:) = pinv(O_sqrt)*Wfm((i-1)*ncv+1:i*ncv,:);
            end
